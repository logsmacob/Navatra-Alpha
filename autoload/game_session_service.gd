...
var run_state: RunStateModel = RunStateModel.new()
var round_state: RoundStateModel = RoundStateModel.new()
...
func _start_round(round_index: int) -> void:
	var quota: int = QuotaService.get_quota_for_round(round_index)
	var hands: int = GameBalanceConfig.get_hands_for_round(round_index)
	var rerolls: int = GameBalanceConfig.BASE_REROLLS_PER_ROUND + run_state.get_extra_rerolls()
...
func spend_reroll() -> bool:
	var spent: bool = round_state.spend_reroll()
	if spent:
		EventBus.round_state_changed.emit(round_state)
	return spent
...
func submit_hand(result: HandResult, values: Array[int]) -> int:
	...
	var score: int = ScoreCalculatorService.calculate(result, values, run_state)
